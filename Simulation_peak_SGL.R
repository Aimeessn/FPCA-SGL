...
################################################################################################################################
# Extract peak measurement for each biomarker for each patient

bm_group = data.frame(bm = c("bm1", "bm2", "bm3", "bm4"), group = c(1,1,2,2))

# Data simulated with Model 1
sim_peak <- list() #a list of `num_sce` lists (one for each simulation scenario), each list is a list of `rep` dfs, each df has `sample_size` rows and 5 cols (ID+4bm)
sim_missing <- list()
for (s in 1:num_sce){
  sim_peak_s <- list() #a list of `rep` dfs, each df has `sample_size` rows and 13 cols, one of this list for each simulation scenario
  sim_missing_s <- numeric(rep)
  for (r in 1:rep){
    bm_peak <- sim_bm_obs[[s]][[r]] %>% dplyr::select(ID, all_of(bm_group$bm)) %>%
      group_by(ID) %>% summarise_all(max, na.rm = TRUE) #"na.rm=T generates -Inf for missing biomarker value
    sim_missing_s[r] <- length(table(is.na(bm_peak)))
    sim_peak_s[[r]] <- bm_peak
  }
  sim_peak <- append(sim_peak, list(sim_peak_s))
  sim_missing[[s]] <- sim_missing_s
}

# Data simulated with Model 2
sim_peak_quad <- list() #a list of `num_sce` lists (one for each simulation scenario), each list is a list of `rep` dfs, each df has `sample_size` rows and 5 cols (ID+4bm)
sim_missing_quad <- list()
for (s in 1:num_sce){
  sim_peak_quad_s <- list() #a list of `rep` dfs, each df has `sample_size` rows and 13 cols, one of this list for each simulation scenario
  sim_missing_quad_s <- numeric(rep)
  for (r in 1:rep){
    bm_peak_quad <- sim_bm_obs_quad[[s]][[r]] %>% dplyr::select(ID, all_of(bm_group$bm)) %>%
      group_by(ID) %>% summarise_all(max, na.rm = TRUE) #"na.rm=T generates -Inf for missing biomarker value
    sim_missing_quad_s[r] <- length(table(is.na(bm_peak_quad)))
    sim_peak_quad_s[[r]] <- bm_peak_quad
  }
  sim_peak_quad <- append(sim_peak_quad, list(sim_peak_quad_s))
  sim_missing_quad[[s]] <- sim_missing_quad_s
}

# Data simulated with Model 3
sim_peak_splines <- list() #a list of `num_sce` lists (one for each simulation scenario), each list is a list of `rep` dfs, each df has `sample_size` rows and 5 cols (ID+4bm)
sim_missing_splines <- list()
for (s in 1:num_sce){
  sim_peak_splines_s <- list() #a list of `rep` dfs, each df has `sample_size` rows and 13 cols, one of this list for each simulation scenario
  sim_missing_splines_s <- numeric(rep)
  for (r in 1:rep){
    bm_peak_splines <- sim_bm_obs_splines[[s]][[r]] %>% dplyr::select(ID, all_of(bm_group$bm)) %>%
      group_by(ID) %>% summarise_all(max, na.rm = TRUE) #"na.rm=T generates -Inf for missing biomarker value
    sim_missing_splines_s[r] <- length(table(is.na(bm_peak_splines)))
    sim_peak_splines_s[[r]] <- bm_peak_splines
  }
  sim_peak_splines <- append(sim_peak_splines, list(sim_peak_splines_s))
...
################################################################################################################################
# Conduct SGL on the peak measurements, with pre-picked alpha=0.05 (~GL), and pick lambda using CV
...
#            a dataframe "bm_df" for simulated biomarkers data (peak measurements)
