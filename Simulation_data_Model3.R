# Simulate datasets with Model 3 (LME with a 3-knot spline function for time) under 5 scenarios (as listed in Table 2) 
# n=2000 and rep=200
...
fixed = data.frame(intercepts = c(0.5, -0.8, 0.2, -0.5),
                   ...
                   b2 = c(-2, 0.5, -1, 1.5),
                   b3 = c(3, -1, -1, 1.5))
tau = c(0.15, 0.3, 0.45) #3 knots from t=0 to t = 0.6
...
weib_haztv <- function(t,alphas,fixed,random_i){
  h0_gamma * t^(h0_gamma-1) * exp(h0_eta) *
    exp(alphas[1]*(fixed$intercepts[1] + fixed$slopes[1]*t + random_i$bm1_intercepts + random_i$bm1_slopes*t +
                     fixed$b1[1]*as.numeric(t>=tau[1])*(t-tau[1]) +
                     fixed$b2[1]*as.numeric(t>=tau[2])*(t-tau[2]) +
                     fixed$b3[1]*as.numeric(t>=tau[3])*(t-tau[3])) +
          alphas[2]*(fixed$intercepts[2] + fixed$slopes[2]*t + random_i$bm2_intercepts + random_i$bm2_slopes*t +
                       fixed$b1[2]*as.numeric(t>=tau[1])*(t-tau[1]) +
                       fixed$b2[2]*as.numeric(t>=tau[2])*(t-tau[2]) +
                       fixed$b3[2]*as.numeric(t>=tau[3])*(t-tau[3])) +
          alphas[3]*(fixed$intercepts[3] + fixed$slopes[3]*t + random_i$bm3_intercepts + random_i$bm3_slopes*t +
                       fixed$b1[3]*as.numeric(t>=tau[1])*(t-tau[1]) +
                       fixed$b2[3]*as.numeric(t>=tau[2])*(t-tau[2]) +
                       fixed$b3[3]*as.numeric(t>=tau[3])*(t-tau[3])) +
          alphas[4]*(fixed$intercepts[4] + fixed$slopes[4]*t + random_i$bm4_intercepts + random_i$bm4_slopes*t +
                       fixed$b1[4]*as.numeric(t>=tau[1])*(t-tau[1]) +
                       fixed$b2[4]*as.numeric(t>=tau[2])*(t-tau[2]) +
                       fixed$b3[4]*as.numeric(t>=tau[3])*(t-tau[3])))
}
...
# Scenario 3: simulate outcome ~ bm group 1 (low cor) + bm group 2 (high cor)
...
t_cen_candidate[1:(num_sce-1)] #0.6878228 0.6498365 0.5714726 0.3251561
  ...
  for (r in 1:rep){
    ...
    bm_true <- sim_bm %>% left_join(random_r %>% mutate(ID = seq(1, sample_size)), by = "ID") %>%
      mutate(bm1 = fixed$intercepts[1] + fixed$slopes[1]*Time + bm1_intercepts + bm1_slopes*Time +
               fixed$b1[1]*as.numeric(Time>=tau[1])*(Time-tau[1]) +
               fixed$b2[1]*as.numeric(Time>=tau[2])*(Time-tau[2]) +
               fixed$b3[1]*as.numeric(Time>=tau[3])*(Time-tau[3]),
             bm2 = fixed$intercepts[2] + fixed$slopes[2]*Time + bm2_intercepts + bm2_slopes*Time +
               fixed$b1[2]*as.numeric(Time>=tau[1])*(Time-tau[1]) +
               fixed$b2[2]*as.numeric(Time>=tau[2])*(Time-tau[2]) +
               fixed$b3[2]*as.numeric(Time>=tau[3])*(Time-tau[3]),
             bm3 = fixed$intercepts[3] + fixed$slopes[3]*Time + bm3_intercepts + bm3_slopes*Time +
               fixed$b1[3]*as.numeric(Time>=tau[1])*(Time-tau[1]) +
               fixed$b2[3]*as.numeric(Time>=tau[2])*(Time-tau[2]) +
               fixed$b3[3]*as.numeric(Time>=tau[3])*(Time-tau[3]),
             bm4 = fixed$intercepts[4] + fixed$slopes[4]*Time + bm4_intercepts + bm4_slopes*Time +
               fixed$b1[4]*as.numeric(Time>=tau[1])*(Time-tau[1]) +
               fixed$b2[4]*as.numeric(Time>=tau[2])*(Time-tau[2]) +
               fixed$b3[4]*as.numeric(Time>=tau[3])*(Time-tau[3]))
...
sim_bm_obs_splines = sim_bm_obs
sim_outcome_splines = sim_outcome

save(sim_bm_obs_splines, sim_outcome_splines, file = "Simulation_survival_splines_datasets.RData")
