# Simulate datasets with Model 2 (LME with a quadratic term in time) under 5 scenarios (as listed in Table 2) 
# n=2000 and rep=200
...
fixed = data.frame(intercepts = c(-0.1, -0.4, -0.2, -0.2),
                   slopes = c(0.1, 0.12, 0.08, 0.07))
...
weib_haztv <- function(t,alphas,fixed,random_i){
  h0_gamma * t^(h0_gamma-1) * exp(h0_eta) *
    exp(alphas[1]*(fixed$intercepts[1] * (t-fixed$slopes[1])^2 + random_i$bm1_intercepts + random_i$bm1_slopes*t) +
          alphas[2]*(fixed$intercepts[2] * (t-fixed$slopes[2])^2 + random_i$bm2_intercepts + random_i$bm2_slopes*t) +
          alphas[3]*(fixed$intercepts[3] * (t-fixed$slopes[3])^2 + random_i$bm3_intercepts + random_i$bm3_slopes*t) +
          alphas[4]*(fixed$intercepts[4] * (t-fixed$slopes[4])^2 + random_i$bm4_intercepts + random_i$bm4_slopes*t))
}
...
for (r in 1:rep){
  ...
  for(i in 1:sample_size){
    sim_unif <- runif(n=1, min=0, max=1)
    cat("r=", r, "i=", i, ": ", sim_unif, "\n")
    surv_time[i] <- try(weib_invsurvtv_numeric(p = sim_unif, alphas = c(0.5, 0.5, 0.5, 0.5),
                                               fixed = fixed, random_i = sim_random[[r]][i,]))
  }
...
set.seed(377)
for (r in 1:rep){
  surv_time <- rweibull(n = sample_size, shape = h0_gamma, scale = 0.4)
...
t_cen_candidate[1:(num_sce-1)] #0.3019574 0.2992821 0.3035782 0.3335405
t_cen = rep(0.3, num_sce)
for (s in 1:(num_sce-1)){
  for (r in 1:rep){
...
sim_bm_obs_quad = sim_bm_obs
sim_outcome_quad = sim_outcome

save(sim_bm_obs_quad, sim_outcome_quad, file = "Simulation_survival_quadratic_datasets.RData")
